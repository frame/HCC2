...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	if (bIsDisabled && m_bShowDisabledBitmap)
	{
		...
		pDC->DrawState(	CPoint(rImage.left - 2, rImage.top), 
	...
	{
		::BitBlt(pDC->m_hDC, rImage.left - 2, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcMem, 0, 0, SRCAND);

		::BitBlt(pDC->m_hDC, rImage.left - 2, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
	} // else
...
void CButtonST::DrawTheText(CDC* pDC, LPCTSTR lpszText, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	pDC->DrawText(lpszText, -1, rpCaption, DT_WORDBREAK | DT_LEFT | DT_CALCRECT);
	rpCaption->OffsetRect(rpItem->left + 4, (centerRect.Height() - rpCaption->Height())/2);
	/* RFU
